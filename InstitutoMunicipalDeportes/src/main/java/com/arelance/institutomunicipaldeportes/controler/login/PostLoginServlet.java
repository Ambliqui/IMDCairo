 ...
 */
package com.arelance.institutomunicipaldeportes.controler.login;
...
public class PostLoginServlet extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        Login usuarioEncontrado = servicesLogin.findLoginByEmail(login);
        login.setIdLogin(usuarioEncontrado.getIdLogin());

        if (usuarioEncontrado.getEmail().equals(email) && usuarioEncontrado.getPassword().equals(password)) {    //Credenciales OK
            request.getSession().setAttribute("usuarioSesion", login);
            request.getRequestDispatcher("PreIndexServlet").forward(request, response);
            return;
        } else if (usuarioEncontrado.getEmail() == null) {      //Email nulo --> no existe el usuario
